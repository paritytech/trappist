...
pub use parachains::{
	ALICE,
	asset_reserve::ASSET_RESERVE_PARA_ID,
	trappist::TRAPPIST_PARA_ID
};
...
pub const INITIAL_BALANCE: u128 = parachains::trappist::INITIAL_BALANCE;
...
mod tests {
	...
	fn reserve_transfer_asset_from_asset_reserve_parachain_to_trappist_parachain() {
		...
		let trappist_remote: MultiLocation = MultiLocation { parents: 1, interior: X1(Parachain(TRAPPIST_PARA_ID)) };

		// 1) Forces XCM default version on Rococo
		// 2) Sends Transact as Superuser to Asset Reserve Parachain to force XCM default version on Asset Reserve Parachain
		Rococo::execute_with(|| {
			...
			let force_default_xcm_version_call = statemine_runtime::RuntimeCall::PolkadotXcm(
				pallet_xcm::Call::<statemine_runtime::Runtime>::force_xcm_version {
		...
		AssetReserveParachain::execute_with(|| {
			...
			statemine_runtime::System::events().iter().for_each(|r| println!(">>> {:?}", r.event));
		...
		Rococo::execute_with(|| { 
			let set_asset_sufficient_call = statemine_runtime::RuntimeCall::Assets(
				pallet_assets::Call::<statemine_runtime::Runtime>::force_asset_status {
					id: Compact(xUSD),
					owner: ALICE.into(),
					issuer: ALICE.into(),
					admin: ALICE.into(),
		...
		// 6) Check the events on Asset Reserve Parachain and validated the XCM supported version is 2
		AssetReserveParachain::execute_with(|| {
			statemine_runtime::System::events().iter().for_each(|r| println!(">>> {:?}", r.event));
 			statemine_runtime::System::assert_has_event(
				pallet_xcm::Event::SupportedVersionChanged(trappist_remote.clone(),2).into(),
...
/* 			let statemine_sovereign_account = parachains::sovereign_account(ASSET_RESERVE_PARA_ID);
			...
			let set_asset_sufficient_call = trappist_runtime::RuntimeCall::Assets(
				pallet_assets::Call::<trappist_runtime::Runtime>::force_asset_status {
					id: Compact(txUSD),
					owner: ALICE.into(),
					issuer: ALICE.into(),
					admin: ALICE.into(),
			...
			trappist_runtime::System::assert_has_event(
				pallet_asset_registry::Event::ReserveAssetRegistered {
					...
					asset_multi_location: MultiLocation {
						parents: 1,
						interior: Junctions::X3(Parachain(ASSET_RESERVE_PARA_ID), PalletInstance(50), GeneralIndex(xUSD.into()))
					},
				}
				.into(),
			);
			trappist_runtime::System::events().iter().for_each(|r| println!(">>> {:?}", r.event));
